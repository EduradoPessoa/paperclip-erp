...
| **PCP** — ordens de produção (`erp_production_orders` + itens/BOM) com ciclo planejada → liberada → em produção → **concluída**; conclusão **consome matéria-prima e produz o acabado** via movimentos de estoque (guarda de disponibilidade, tudo-ou-nada) | ✅ Ciclo 8 (PCP) |
| **Serviços** — ordens de serviço (`erp_service_orders` + itens) com **agendamento** e **SLA** (deadline, slaMet); conclusão **cria o receivable** (faturamento) vinculado ao cliente | ✅ Ciclo 9 (Serviços) |
| **TMS** — ordens de frete (`erp_freight_orders`) com transportadora, origem/destino, **agendamento de coleta**, custo do frete, **rastreio append-only** (eventos de tracking; `delivered` encerra) e **vínculo com CT-e** (Fiscal, modelo validado) | ✅ Ciclo 10 (TMS) |
| **Importação** — ordens de importação (`erp_import_orders` + itens) com **documento (DI/DUIMP)** e chegada; **desembaraço faz rateio dos custos** (frete+seguro proporcionais ao valor) → atualiza custo dos produtos e **cria payable** do fornecedor | ✅ Ciclo 11 (Importação) |
| **Exportação** — ordens de exportação (`erp_export_orders` + itens) com **declaração**, **incoterm** e **moeda**; embarque **fatura o cliente** (receivable na moeda da ordem, câmbio em metadata) | ✅ Ciclo 12 (Exportação) |
| **OMS** — consolidação de pedidos **multi-canal** (`erp_oms_orders`: canal, id externo, **promessa de entrega**); confirmação **cria o pedido de venda** (Vendas) quando há cliente + itens com produto → fluxo segue para faturamento/fiscal | ✅ Ciclo 13 (OMS) |
| **Contabilidade** — **partidas dobradas** (`erp_journal_entries` + lines) sobre o plano de contas: lançamentos **balanceados** (débitos = créditos, check constraint por linha), numeração sequencial por empresa, competência (entryDate) e ciclo draft → **postado** → revertido/cancelado | ✅ Ciclo 14 (Contabilidade) |
| **Ativo Imobilizado** — fichas de ativo (`erp_fixed_assets`) com aquisição, vida útil e valor residual; **depreciação linear** por competência (`erp_depreciation_runs` append-only, último passo clampa no residual) e **baixa** com valor de alienação | ✅ Ciclo 15 (Ativo Imobilizado) |
| **Custo** — **centros de custo** (`erp_cost_centers`) + **alocações por período** (`erp_cost_allocations`, com origem: OP/importação/financeiro) e **custo de produção** (matéria-prima Σ qtd×custo + mão de obra/rateio via alocações da OP) | ✅ Ciclo 16 (Custo) |
| **Memória M2** — contrato `MemoryProvider` (capture/query + capabilities), registry de providers e provider local **`local_trail`** (zero-config); **hooks de execução** `pre_run_hydrate` (query) e `post_run_capture` (captura com resumo) ligados aos runs com atribuição (agente + runId) e trilha auditada | ✅ Ciclo 17 (Memória M2) |
| **RBAC por módulo (enforcement)** — permissões `erp:<módulo>:<ação>` aplicadas: validator de grants aceita chaves `erp:` (aditivo), helper `assertErpPermission` (board passa = humano no controle; agente exige grant ativo por empresa) aplicado no **registro de módulos** (`erp:modules:manage`) e no **Compras** (`erp:purchasing:manage`) como referência | ✅ Ciclo 18 (RBAC enforcement) |
